...
# Final Project User story 7
describe 'Merchant Invoice Show Page', type: :feature do
  before :each do
    @merchant = create(:merchant)
    @customer = create(:customer)
    @coupon = create(:coupon, merchant: @merchant, discount_type: 'percent', amount: 10)
    @invoice = create(:invoice, customer: @customer, coupon: @coupon, status: 'completed')
    @item1 = create(:item, merchant: @merchant)
    @item2 = create(:item, merchant: @merchant)
    @invoice_item1 = create(:invoice_item, invoice: @invoice, item: @item1, quantity: 2, unit_price: 1000) # $10.00
    @invoice_item2 = create(:invoice_item, invoice: @invoice, item: @item2, quantity: 1, unit_price: 2000) # $20.00
  ...
  it 'shows the subtotal and grand total for the invoice' do
    ...
    expect(page).to have_content('$40.00') # $30.00 + $10.00
    expect(page).to have_content('Grand Total:')
    expect(page).to have_content('$36.00') # $40.00 - 10% discount
  end
